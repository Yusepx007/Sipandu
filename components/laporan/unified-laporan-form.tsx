...
function WargaSelectBox({ label, name, required, wargaList, value, onChange }: {
  label: string; name: string; required?: boolean
  wargaList: Partial<Warga>[]; value?: string; onChange?: (v: string) => void
}) {
  return (
    <div>
      <label htmlFor={name} className={labelClass}>
...
export function UnifiedLaporanForm({ wargaList, rtList }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      else { setSuccess(true); setTimeout(() => router.push('/rt'), 2500) }
    } catch {
  ...
  if (success) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center gap-4 bg-white rounded-2xl border border-gray-100 shadow-sm">
        ...
        <p className="text-gray-500 text-sm">Laporan dikirim ke Admin dan menunggu verifikasi RW</p>
    ...
    <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-5">
      ...
      {jenis === 'masuk' && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Field label="Alamat di RT ini" name="alamat" placeholder="Alamat lengkap" required />
      ...
      {jenis === 'keluar' && (
        <div className="space-y-4">
          <WargaSelectBox label="Pilih Warga" name="warga_id" required wargaList={wargaList} />
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      {/* ── Keterangan (selalu muncul kalau jenis sudah dipilih) ── */}
      {jenis && (
      ...
      {jenis && (
        <div className="px-4 py-3 rounded-xl bg-amber-50 border border-amber-200 text-amber-700 text-xs">
          <strong>Catatan:</strong> Laporan akan berstatus <em>Diajukan</em> dan perlu diverifikasi Ketua RW sebelum masuk statistik resmi.
        </div>
      )}
      ...
      <div className="flex gap-3 pt-2 border-t border-gray-100">
        ...
        <button id="btn-submit-laporan" type="submit" disabled={pending || !jenis}
